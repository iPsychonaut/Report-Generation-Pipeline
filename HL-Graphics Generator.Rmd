...
gen_graphics_from_row <- function(row) {
  ...
  sample_species = row$Species_of_Origin 
  ...
  donut_plot<- layout(donut_plot, annotations=list(x = 0.5, y = 0.56, text = paste(known_mg_g_sum,'mg/g',sep=''),
  ...
  donut_plot<- layout(donut_plot, annotations=list(x = 0.5, y = 0.42, text = paste(other_mg_g_sum,'mg/g',sep=''),
  ...
  sample_table <- plot_ly(
    ...
    header = list(
      ...
      height = 50),
    cells = list(
      values = cbind(c('CULTIVATOR/PRODUCER:',sample_client),c('SPECIES:',sample_species),
                     c('CULTIVAR:', sample_cultivar),c('GENERATION DATE:', sample_gen_date)),
      align = c('right','left'),
      line = list(width = 1, color = 'black'),
